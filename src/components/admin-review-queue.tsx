          ...
          {submissions.map((submission) => (
            <Card key={submission.id} className="rounded-none">
              ...
              <CardContent className="grid gap-6 lg:grid-cols-[1fr_320px]">
                ...
                <div className="space-y-3"><Textarea value={reasons[submission.id] || ""} onChange={(event) => setReasons((current) => ({ ...current, [submission.id]: event.target.value }))} placeholder="Review note or required rejection reason" /><div className="grid grid-cols-2 gap-2"><Button onClick={() => decide(submission.id, "published")} disabled={busy === submission.id}><CheckCircle2 /> Publish</Button><Button variant="outline" onClick={() => decide(submission.id, "rejected")} disabled={busy === submission.id}><XCircle /> Reject</Button></div></div>
              </CardContent>
            </Card>
          ))}
        </div>
